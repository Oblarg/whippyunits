...
use whippyunits_lsp_proxy::{DisplayConfig, LspProxy};
...
async fn main() -> Result<()> {
    ...
    info!(
        "Display config: verbose={}, unicode={}",
        display_config.verbose, display_config.unicode
    );
    ...
    let mut ra_stdin = rust_analyzer
        .stdin
        .take()
        .expect("Failed to get rust-analyzer stdin");
    let ra_stdout = rust_analyzer
        .stdout
        .take()
        .expect("Failed to get rust-analyzer stdout");
    let ra_stderr = rust_analyzer
        .stderr
        .take()
        .expect("Failed to get rust-analyzer stderr");
...
fn find_rust_analyzer() -> Result<String> {
    ...
    Err(anyhow::anyhow!(
        "rust-analyzer not found. Please ensure it's installed and in your PATH."
    ))
}
...
async fn spawn_rust_analyzer(path: &str, args: &[String]) -> Result<Child> {
    ...
    info!(
        "  DEVELOPER_DIR: {}",
        std::env::var("DEVELOPER_DIR").unwrap_or_default()
    );
    info!(
        "  SDKROOT: {}",
