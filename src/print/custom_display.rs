    ...
    T: Copy + Into<f64>,
{
    fn format_as(&self, unit: &str) -> Result<String, String> {
        ...
        let target_unit_info =
            lookup_unit_literal(unit).ok_or_else(|| format!("Unknown unit: {}", unit))?;
        ...
        let conversion_factor = calculate_conversion_factor::<SCALE_P2, SCALE_P3, SCALE_P5, SCALE_PI>(
            unit,
            target_unit_info,
        );
