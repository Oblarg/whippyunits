...
use whippyunits_default_dimensions::{DIMENSION_LOOKUP, SI_PREFIXES, BASE_UNITS, COMPOUND_UNITS, UNIT_LITERALS};
...
fn is_valid_compound_unit(unit: &str) -> bool {
    COMPOUND_UNITS.iter().any(|compound_unit_info| compound_unit_info.symbol == unit)
}

/// Check if a string is a valid unit literal (like min, h, hr, d, etc.)
fn is_valid_unit_literal(unit: &str) -> bool {
    UNIT_LITERALS.iter().any(|unit_literal_info| unit_literal_info.symbol == unit)
}
...
pub fn get_unit_dimensions(unit_name: &str) -> (i16, i16, i16, i16, i16, i16, i16, i16, i16, i16, i16, i16) {
    ...
    if let Some(unit_literal_info) = UNIT_LITERALS.iter().find(|info| info.symbol == unit_name) {
        ...
        return (mass, length, time, current, temp, amount, lum, angle, p2, p3, p5, pi);
    }
