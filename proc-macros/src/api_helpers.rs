use whippyunits_core::{Dimension, SiPrefix, dimension_exponents::DynDimensionExponents};
...
pub fn parse_unit_with_prefix(unit_name: &str) -> (Option<&'static SiPrefix>, String) {
    ...
    if let Some((prefix, base)) = SiPrefix::strip_any_prefix_symbol(unit_name) {
        // Check if the base unit exists
        if Dimension::find_unit_by_symbol(base).is_some() {
            return (Some(prefix), base.to_string());
        }
    }
...
pub fn lookup_unit_literal(unit_name: &str) -> Option<(&'static Dimension, &'static whippyunits_core::Unit)> {
...
pub fn get_units_by_exponents(exponents: (i16, i16, i16, i16, i16, i16, i16, i16)) -> Vec<(&'static Dimension, &'static whippyunits_core::Unit)> {
